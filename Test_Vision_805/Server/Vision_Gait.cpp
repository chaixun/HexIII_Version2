...
int RobotTurn(Robots::ROBOT_BASE *pRobot, const Robots::GAIT_PARAM_BASE *pParam)
{
    ...
    double pEE[18], pBodyPE_1[18], pBodyPE[6];

    CalPee(pRealParam->count, pRealParam->turndata, 0.05, pEE, pBodyPE_1);

    memcpy(pBodyPE, pBodyPE_1 + 3, 3*sizeof(double));
    memcpy(pBodyPE + 3, pBodyPE_1, 3*sizeof(double));
...
int RobotMove(Robots::ROBOT_BASE *pRobot, const Robots::GAIT_PARAM_BASE *pParam)
{
    ...
    if(pRealParam->count < halfcounter)
    {
        ...
        pBodyPE[0] += Ellipse_ax*(1-cos(s))/2;
        pBodyPE[2] += Ellipse_az*(1-cos(s))/2;
    }
    else
    {
        ...
        pBodyPE[0] += Ellipse_ax + Ellipse_ax*(1-cos(s))/2;
        pBodyPE[2] += Ellipse_az + Ellipse_az*(1-cos(s))/2;
    }
...
int RobotBody(Robots::ROBOT_BASE *pRobot, const Robots::GAIT_PARAM_BASE *pParam)
{
    ...
    if(pRealParam->bodymovedata[1] < 0)
    {
        pBodyPE[1] = 0.2;
    }
    ...
    pBodyPE[0] += pRealParam->bodymovedata[0] * (1 - cos(s))/2;
    pBodyPE[1] += pRealParam->bodymovedata[1] * (1 - cos(s))/2;
    pBodyPE[2] += pRealParam->bodymovedata[2] * (1 - cos(s))/2;
